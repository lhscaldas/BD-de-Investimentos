...
class ResumoView(LoginRequiredMixin, ListView):
    ...
    #     """Retorna os ativos do usuário e adiciona `valor_atualizado` e rentabilidades a partir do JSON."""
    #     ativos_queryset = Ativo.objects.filter(usuario=self.request.user)
    #     ativos = list(ativos_queryset)  # Converte para lista para manipulação
    #     dados_financeiros = self.carregar_dados_financeiros()

    #     if self.request.user.username not in dados_financeiros:
    #         return ativos_queryset  # Retorna o queryset original se não houver dados financeiros

    #     for ativo in ativos:
    #         ativo_dados = dados_financeiros[self.request.user.username].get(str(ativo.id), {})

    #         # Adiciona dinamicamente os atributos para evitar erro de atributo inexistente
    #         setattr(ativo, "valor_atualizado", ativo.valor_inicial)
    #         setattr(ativo, "rentabilidade_1m_perc", 0)
    #         setattr(ativo, "rentabilidade_1a_perc", 0)
    #         setattr(ativo, "rentabilidade_total_perc", 0)

    #         # Define o valor atualizado como o último valor encontrado no JSON
    #         if "valor" in ativo_dados and ativo_dados["valor"]:
    #             ultimo_mes = max(ativo_dados["valor"].keys())  # Obtém a última chave de mês
    #             setattr(ativo, "valor_atualizado", ativo_dados["valor"][ultimo_mes])

    #         # Obtém a penúltima rentabilidade mensal, se disponível
    #         if "rentabilidade" in ativo_dados and len(ativo_dados["rentabilidade"]) > 1:
    #             penultimo_mes = sorted(ativo_dados["rentabilidade"].keys())[-2]
    #             setattr(ativo, "rentabilidade_1m_perc", ativo_dados["rentabilidade"][penultimo_mes])

    #     return ativos_queryset.model.objects.filter(id__in=[ativo.id for ativo in ativos])  # Retorna um queryset válido
